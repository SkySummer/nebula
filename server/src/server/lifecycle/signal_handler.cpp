...
bool SignalHandler::handle_signal_event(unsigned char signal, const std::function<void()>& stop_callback) noexcept {
    ...
    common::Logger::instance()
        ...
        .field("signal", value)
        .field("signal_name", signal_name(value))
...
bool SignalHandler::drain_signal_pipe(const std::stop_token& stop) noexcept {
...
void SignalHandler::start(std::function<void()> stop_callback) {
    ...
    try {
        thread_ = std::jthread([this](const std::stop_token& stop) { wait_loop(stop); });
...
void SignalHandler::wait_loop(const std::stop_token& stop) noexcept {
