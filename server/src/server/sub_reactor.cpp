...
SubReactor::SubReactor(std::size_t id, const app::ServerConfig& config, RequestDispatcher request_dispatcher,
...
bool SubReactor::start() {
    ...
    try {
        thread_ = std::thread(&SubReactor::run_loop, this);
    } catch (const std::exception& e) {
        run_state_.store(RunState::Drained);
        {
            std::lock_guard lock(wakeup_mutex_);
