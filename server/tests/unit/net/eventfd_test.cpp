...
void test_notify_eventfd_retries_on_eintr_then_succeeds() {
    ...
    test::expect_true(event_fd >= 0, "eventfd should be created");
    ...
    test::expect_equal(err, 0, "notify_eventfd should retry EINTR and succeed");
    test::expect_equal(fault_state().write_call_count, std::size_t{2},
                       "notify_eventfd should invoke write twice after EINTR retry");

    std::uint64_t counter = 0;
    const ssize_t read_n = ::read(guard.get(), &counter, sizeof(counter));
    test::expect_equal(read_n, static_cast<ssize_t>(sizeof(counter)), "eventfd should contain one wakeup");
    test::expect_equal(counter, std::uint64_t{1}, "eventfd counter should increment once");
}
...
void test_notify_eventfd_treats_eagain_as_success() {
    ...
    test::expect_equal(err, 0, "notify_eventfd should treat EAGAIN as success");
    test::expect_equal(fault_state().write_call_count, std::size_t{1}, "notify_eventfd should not retry after EAGAIN");
}
...
void test_notify_eventfd_propagates_nonrecoverable_error() {
    ...
    const int err = nebula::net::notify_eventfd(-1);
    test::expect_equal(err, EBADF, "notify_eventfd should return nonrecoverable errno");
    test::expect_equal(fault_state().write_call_count, std::size_t{1},
...
void test_drain_eventfd_retries_on_eintr_and_stops_on_eagain() {
    ...
    test::expect_equal(err, 0, "drain_eventfd should continue after EINTR and stop on EAGAIN");
    test::expect_equal(fault_state().read_call_count, std::size_t{3},
...
void test_drain_eventfd_propagates_nonrecoverable_error() {
    ...
    const int err = nebula::net::drain_eventfd(-1);
    test::expect_equal(err, EBADF, "drain_eventfd should return nonrecoverable errno");
    test::expect_equal(fault_state().read_call_count, std::size_t{1},
...
void test_drain_eventfd_real_fd_empty_is_success() {
    ...
    test::expect_equal(err, 0, "drain_eventfd should succeed when real eventfd is empty");
    test::expect_equal(fault_state().read_call_count, std::size_t{1},
                       "drain_eventfd should perform one read on empty real eventfd");
}

int run_eventfd_tests() {
...
int main() {
    return nebula::test::run_main(run_eventfd_tests);
}
