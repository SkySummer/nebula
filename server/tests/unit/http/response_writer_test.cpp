...
void test_plain_text_default_header_canonical() {
    const http::HttpResponse response = nebula::http::make_plain_text_response(http::HttpStatus::OK, "hi");
    const std::string serialized = nebula::http::serialize_http_response(response, true);
...
void test_redirect_response_sets_location_header() {
    const http::HttpResponse response = nebula::http::make_redirect_response(http::HttpStatus::Found, "/login");

    const std::string serialized = nebula::http::serialize_http_response(response, true);
    test::expect_contains(serialized, "HTTP/1.1 302 Found\r\n", "redirect response should keep requested status");
...
void test_suppress_body_keeps_content_length() {
    ...
    test::expect_not_contains(serialized, "head-body", "suppressed response should not include body bytes");
    test::expect_true(serialized.ends_with("\r\n\r\n"), "suppressed response should end after headers");
}

void test_informational_status_suppresses_body_automatically() {
    http::HttpResponse response;
    response.status = http::HttpStatus::EarlyHints;
    response.body = "hint-body";

    const std::string serialized = nebula::http::serialize_http_response(response, true);
    test::expect_contains(serialized, "HTTP/1.1 103 Early Hints\r\n", "status line should be preserved");
    test::expect_contains(serialized, "\r\nContent-Length: 0\r\n", "1xx response should force zero content-length");
    test::expect_not_contains(serialized, "hint-body", "1xx response should suppress body automatically");
    test::expect_true(serialized.ends_with("\r\n\r\n"), "1xx response should end after headers");
}

void test_no_content_status_suppresses_body_automatically() {
    http::HttpResponse response;
    response.status = http::HttpStatus::NoContent;
    response.body = "unexpected";

    const std::string serialized = nebula::http::serialize_http_response(response, true);
    test::expect_contains(serialized, "HTTP/1.1 204 No Content\r\n", "status line should be preserved");
    test::expect_contains(serialized, "\r\nContent-Length: 0\r\n", "204 response should force zero content-length");
    test::expect_not_contains(serialized, "unexpected", "204 response should suppress body automatically");
...
void test_not_modified_status_suppresses_body_automatically() {
    ...
    const std::string serialized = nebula::http::serialize_http_response(response, true);
    test::expect_contains(serialized, "HTTP/1.1 304 Not Modified\r\n", "status line should be preserved");
    test::expect_contains(serialized, "\r\nContent-Length: 0\r\n", "304 response should force zero content-length");
    test::expect_not_contains(serialized, "cached", "304 response should suppress body automatically");
    test::expect_true(serialized.ends_with("\r\n\r\n"), "304 response should end after headers");
}

int run_response_writer_tests() {
...
int main() {
    return nebula::test::run_main(run_response_writer_tests);
}
