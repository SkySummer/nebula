...
void test_parse_chunked_body() {
    const std::string raw =
        ...
        "5\r\nhello\r\n6\r\n world\r\n0\r\n\r\n";
    const auto parsed = nebula::http::parse_http_request(raw, kMaxHeader, kMaxBody, kMaxRequestTarget);
    expect_equal(parsed.status, ParseStatus::Complete, "chunked request should parse");
    expect_equal(parsed.request.body, std::string("hello world"), "chunked body should decode");
}
...
void test_parse_chunked_body_with_extension_and_trailer() {
    const std::string raw =
        ...
        "a;foo=bar\r\n0123456789\r\n0\r\nX-Trace: 1\r\nX-Tag: ok\r\n\r\n";
    const auto parsed = nebula::http::parse_http_request(raw, kMaxHeader, kMaxBody, kMaxRequestTarget);
    expect_equal(parsed.status, ParseStatus::Complete, "chunked request with extension and trailer should parse");
    expect_equal(parsed.request.body, std::string("0123456789"), "chunked body should decode with trailer present");
    expect_true(!parsed.request.headers.contains("x-trace"), "chunk trailer should not be exposed as request header");
}
...
void test_parse_chunked_need_more() {
    const std::string raw =
        "POST /echo HTTP/1.1\r\nHost: localhost\r\nTransfer-Encoding: chunked\r\n\r\n4\r\ntest\r\n0\r\nX-T: ok\r\n";
    const auto parsed = nebula::http::parse_http_request(raw, kMaxHeader, kMaxBody, kMaxRequestTarget);
    expect_equal(parsed.status, ParseStatus::NeedMore, "incomplete chunk trailer terminator should need more data");
}
...
void test_parse_chunked_incremental_context_across_fragments() {
    ...
    for (std::size_t idx = 0; idx < fragment_sizes.size(); ++idx) {
        ...
        if (idx + 1U < fragment_sizes.size()) {
            expect_equal(parsed.status, ParseStatus::NeedMore,
                         "chunked incremental parsing should keep requesting more data");
            continue;
        }

        expect_equal(parsed.status, ParseStatus::Complete, "chunked incremental parsing should complete");
        expect_equal(parsed.request.body, std::string("abc"), "chunked incremental parsing should decode once");
    }
}
...
void test_parse_context_resets_after_complete_for_pipeline() {
    ...
    const auto first_parsed = nebula::http::parse_http_request(combined, kMaxHeader, kMaxBody,
                                                               std::numeric_limits<std::size_t>::max(), context);
    expect_equal(first_parsed.status, ParseStatus::Complete, "first request should parse with incremental context");
    expect_equal(first_parsed.request.body, std::string("hello"), "first request body should decode");
    expect_equal(first_parsed.consumed_bytes, first.size(), "first request should consume only first message");
    ...
    const auto second_parsed = nebula::http::parse_http_request(second_view, kMaxHeader, kMaxBody,
                                                                std::numeric_limits<std::size_t>::max(), context);
    expect_equal(second_parsed.status, ParseStatus::Complete,
                 "context should reset automatically for pipelined request");
    expect_equal(second_parsed.request.method, HttpMethod::Get, "second request method should parse");
    expect_equal(second_parsed.request.path, std::string("/next"), "second request path should parse");
}

void test_parse_chunked_context_releases_decoded_capacity_after_complete() {
    const std::string body(static_cast<std::size_t>(64U) * 1024U, 'x');
    const std::string raw = std::format(
        "POST /echo HTTP/1.1\r\nHost: localhost\r\nTransfer-Encoding: chunked\r\n\r\n{:x}\r\n{}\r\n0\r\n\r\n",
        body.size(), body);
    ...
    expect_equal(parsed.status, ParseStatus::Complete, "large chunked request should parse");
    expect_equal(parsed.request.body, body, "large chunked body should decode");
    expect_true(context.decoded_chunked_body.capacity() < body.size(),
...
void test_parse_context_resets_after_error_for_reuse() {
    ...
    expect_parse_error(broken_parsed, HttpStatus::BadRequest, "invalid chunk size should return parse error");
    expect_true(!context.header_parsed, "context should reset header parsed flag after error");
    expect_true(!context.chunked_body, "context should reset chunked flag after error");
    expect_equal(context.header_bytes, 0U, "context should reset header bytes after error");
    expect_equal(context.chunk_cursor, 0U, "context should reset chunk cursor after error");
    expect_equal(context.chunk_size, 0U, "context should reset chunk size after error");
    expect_equal(context.chunk_phase, nebula::http::ChunkedDecodePhase::ChunkSizeLine,
    ...
    expect_equal(next_parsed.status, ParseStatus::Complete, "context should be reusable right after parse error");
    expect_equal(next_parsed.request.method, HttpMethod::Get, "request method should parse after reset");
    expect_equal(next_parsed.request.path, std::string("/next"), "request path should parse after reset");
}
...
void test_parse_chunked_with_content_length_rejected() {
    const std::string raw =
        "POST /echo HTTP/1.1\r\nHost: localhost\r\nTransfer-Encoding: chunked\r\nContent-Length: 4\r\n\r\n";
    const auto parsed = nebula::http::parse_http_request(raw, kMaxHeader, kMaxBody, kMaxRequestTarget);
    expect_parse_error(parsed, HttpStatus::BadRequest, "content-length with transfer-encoding should fail");
}

void test_parse_transfer_encoding_unsupported() {
    const std::string raw = "POST /echo HTTP/1.1\r\nHost: localhost\r\nTransfer-Encoding: gzip\r\n\r\n";
    const auto parsed = nebula::http::parse_http_request(raw, kMaxHeader, kMaxBody, kMaxRequestTarget);
    expect_parse_error(parsed, HttpStatus::NotImplemented, "unsupported transfer-encoding should return 501");
}

void test_parse_invalid_chunked_body() {
    const std::string raw = "POST /echo HTTP/1.1\r\nHost: localhost\r\nTransfer-Encoding: chunked\r\n\r\nz\r\n";
    const auto parsed = nebula::http::parse_http_request(raw, kMaxHeader, kMaxBody, kMaxRequestTarget);
    expect_parse_error(parsed, HttpStatus::BadRequest, "invalid chunk-size should fail");
}

void test_parse_chunked_size_with_leading_whitespace_rejected() {
    const std::string raw = "POST /echo HTTP/1.1\r\nHost: localhost\r\nTransfer-Encoding: chunked\r\n\r\n a\r\n";
    const auto parsed = nebula::http::parse_http_request(raw, kMaxHeader, kMaxBody, kMaxRequestTarget);
    expect_parse_error(parsed, HttpStatus::BadRequest, "chunk-size with leading whitespace should fail");
}
...
void test_parse_chunked_size_with_whitespace_before_extension_rejected() {
    const std::string raw =
        "POST /echo HTTP/1.1\r\nHost: localhost\r\nTransfer-Encoding: chunked\r\n\r\na ;foo=bar\r\n";
    const auto parsed = nebula::http::parse_http_request(raw, kMaxHeader, kMaxBody, kMaxRequestTarget);
    expect_parse_error(parsed, HttpStatus::BadRequest, "chunk-size with whitespace before extension should fail");
}
...
void test_parse_chunked_content_too_large() {
    const std::string raw =
        "POST /echo HTTP/1.1\r\nHost: localhost\r\nTransfer-Encoding: chunked\r\n\r\n6\r\n123456\r\n0\r\n\r\n";
    const auto parsed = nebula::http::parse_http_request(raw, kMaxHeader, 5U, kMaxRequestTarget);
    expect_parse_error(parsed, HttpStatus::ContentTooLarge, "decoded chunked payload exceeding limit should fail");
}
...
void test_parse_chunked_consumed_bytes_for_pipeline() {
    ...
    expect_equal(parsed.request.body, std::string("hello"), "chunked request body should decode");
    expect_equal(parsed.consumed_bytes, first.size(), "consumed bytes should include full chunked request bytes");
}
